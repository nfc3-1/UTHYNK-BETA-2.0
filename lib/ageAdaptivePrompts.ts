...
const categoryContexts: Record<string, { child: string; teen: string }> = {
  "Applied Ethics": {
...
const titleFrameMap: Record<string, PromptFrame> = {
...
function contextFor(category: string, ageBand: AgeBand) {
...
export function normalizeAgeBand(value?: string | null): AgeBand {
...
export function ageBandLabel(ageBand: AgeBand) {
...
function inferFrame(question: string, index = 0): PromptFrame {
  ...
  const fallbackFrames: PromptFrame[] = [
    "evidence",
    "incentive",
    "assumption",
    "opposing",
...
function promptForFrame(frame: PromptFrame, category: string, ageBand: AgeBand) {
...
export function adaptQuestionForAge(
  question: string,
  category: string,
